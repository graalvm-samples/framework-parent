...
public class ContentTypeMap {
    ...
    private static String conentTypeApplicationOctetStream="application/octet-stream";
    static {
...
//        map.put(".vst"	,"application/x-vst");
    ...
    public static String getContentType(String filenameExtension){
        if(StringUtils.isEmpty(filenameExtension)){
            throw new DynamicBaseException("file_name_is_empty","文件名称为空");
        }
        int last=filenameExtension.lastIndexOf(".");
        if(last==-1){
            return conentTypeApplicationOctetStream;
        }
        String extension=filenameExtension.substring(last,filenameExtension.length());
        System.out.println("extension："+extension);
        if(StringUtils.isEmpty(extension)){
            return conentTypeApplicationOctetStream;
        }
        String conentType=map.get(extension);
        if(StringUtils.isEmpty(conentType)){
            return conentTypeApplicationOctetStream;
        }
        return conentType;
    }
